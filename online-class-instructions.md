...
- **Passive attendance** — camera on, doing something else
- **Early dropout** — first 15 minutes feel slow, student mentally leaves
- **Invisible struggle** — student is stuck but will not type in chat or unmute
- **Social isolation** — no hallway moments, no organic relationship-building, motivation drops by week 4
- **Tool friction as exit ramp** — a login issue or broken link becomes a reason to disengage entirely

Every structural decision below addresses at least one of these failure modes.

---

### Platform Infrastructure

**Primary delivery:** Zoom or Google Meet. Zoom is preferred for breakout room control and polling integrations.
...
- Camera on is the default class norm, established in session 1 and maintained consistently
- Second browser tab open to the class Discord server or class Slack throughout every session
- Stable internet; students on unstable connections are given pre-downloaded starter files as backup

**Async infrastructure between sessions:**
- Class Discord server (or Google Classroom if institution requires it) with these channels: `#show-your-build`, `#stuck-help`, `#prompt-of-the-week`, `#random`
- `#show-your-build` is the highest-priority channel. Every student posts their session output URL here before the next class. This is not optional — it is part of the completion check.
- Instructor or TA responds to every post in `#show-your-build` with one specific comment, not generic praise. "The color contrast on your buttons makes the text hard to read — try prompting for higher contrast" is useful. "Great job!" is not.
...
**Worst prompt competition:** Instructor shows a deliberately terrible prompt ("make a website"). Students race to type the worst possible prompt they can imagine in chat. Instructor picks the funniest and runs it live. Class sees the output.

**Build speedrun:** Students have 4 minutes to prompt AI into producing anything — the most absurd result wins. No judgment. Students post URLs in chat. Class votes with emoji reactions.

**Trivia about the day's tool:** Three rapid-fire questions about what they will use today. First correct answer in chat wins a point on the class leaderboard (see gamification section below).

**Bug hunt:** Instructor shares screen with a broken app. Students type what they think is wrong before the explanation begins. No wrong answers — all guesses are read aloud.

---

### Engagement Mechanics Embedded in Session Structure

**Chat as a parallel track, not a backchannel.**
Students should be typing in chat throughout instruction, not just during designated interaction moments. Instructor reads chat while speaking. This requires practice — instructors who are not used to monitoring chat while talking need to rehearse it. The alternative is that chat goes silent, which signals to students that their responses do not matter.

Techniques:
- "Drop your answer in chat, don't wait to be called on" — said explicitly, every time a question is asked
- "I'm reading chat right now" — said aloud periodically so students know it is being watched
- Call out specific chat responses by name: "Kai said X — that's exactly the right instinct, here's why"
...
The week between sessions is where momentum dies if nothing bridges the gap. Middle schoolers have short memory for low-urgency tasks and no intrinsic reason to revisit the class material unless something pulls them back.

**Prompt of the Week:**
Posted in `#prompt-of-the-week` within 24 hours of each session. One challenge prompt, optional, takes 10–15 minutes. Example: "Prompt AI to build a page that shows today's weather for any city the user types in. Post your URL." Students who complete it get a leaderboard point. No grade attached — grade attachment kills intrinsic motivation for optional work.

**Show Your Build posts:**
Required within 48 hours of each session. Students post their session deliverable URL in `#show-your-build`. Instructor responds specifically to every post. Students are expected to leave one emoji reaction on at least two classmates' posts. This is the minimum viable async social layer.
...
| Action | Points |
|--------|--------|
| Post session deliverable in `#show-your-build` on time | 1 |
| Complete the Prompt of the Week | 2 |
| Win the class vote in any Energy Open activity | 1 |
| Leave specific feedback (not just an emoji) on a classmate's build | 1 |
| Instructor spotlight feature | 3 |
| Identify an AI error during session and explain it in chat | 1 |
| Submit all capstone documents before deadline | 3 |

**Display:** Leaderboard posted in Discord at the start of each session as a screenshot. Not announced verbally — students will find it. This generates organic pre-session engagement.

**Rewards:** Non-grade. Options: custom Discord role ("Builder of the Week"), choosing the Energy Open activity for the next session, presenting their build first on Demo Day (some students want this, others do not). Keep rewards low-stakes and preference-based.

The leaderboard is designed so a student who never wins the Energy Open can still lead by posting consistently and leaving good feedback. Consistent engagement beats performance.
...
- After every three presentations, a 2-minute break with camera-optional. No instruction. Students stretch, get water, return.
- The non-graded class vote runs via live poll (Mentimeter or Slido) at the end. Results displayed live. This is the highest-engagement moment of the session — put it last.
...
**Use names constantly.** "Great point" addresses no one. "Rafael, that prompt structure is exactly what we covered in session 3" addresses Rafael and signals to every other student that they are being seen individually.

**Do not ask "does everyone understand?"** This question produces universal silence online. Replace with: "Type one thing that confused you" or "Drop a thumbs up if you know what to build next, thumbs down if not." Binary or low-stakes responses get responses. Open-ended questions in a large online room get silence.

**Keep your camera on and your face expressive.** Students read instructor body language even through a small window. A flat affect on screen reads as indifference. Overexpression online reads as normal engagement.

**Treat tech issues as public, not private.** When a student's Replit crashes or Lovable won't load, say it aloud: "Kai's running into a login issue — here's the backup link, Kai." Other students who have the same issue will not ask unless they see it handled without penalty.